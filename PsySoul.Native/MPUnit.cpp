...
MPUnit::MPUnit(unsigned len)
{
	length = len;
	value = new unsigned short[len];
}

MPUnit::MPUnit(const MPUnit &n)
{
	length = n.length;
...
void MPUnit::operator = (const MPUnit &n)
{
	unsigned i;
...
void MPUnit::operator = (unsigned short n)
{
	value[0] = n;
	unsigned i;
	for (i = 1; i < length; i++)
...
unsigned short MPUnit::remainder(unsigned short n)
{
	unsigned i = length;
	unsigned rem = 0;
	while (i-- != 0)
...
int MPUnit::Compare(const MPUnit &n) const
{
	unsigned i;
	if (length > n.length)
	{
...
int MPUnit::Compare(unsigned short n) const
{
	unsigned i;
...
void MPUnit::shift(unsigned bit)
{
...
void MPUnit::Divide(const MPUnit &dividend, const MPUnit &divisor, MPUnit &quotient,
	MPUnit &remainder)
{
	if (divisor.IsZero())
		numeric_overflow();
...
void MPUnit::operator /= (const MPUnit &n)
{
	MPUnit remainder(n.length);
	MPUnit dividend(*this);
	Divide(dividend, n, *this, remainder);
}

void MPUnit::operator %= (const MPUnit &n)
{
	MPUnit quotient(length);
	MPUnit dividend(*this);
	Divide(dividend, n, quotient, *this);
}

void MPUnit::Power(const MPUnit &base, const MPUnit &exponent,
	const MPUnit &modulus, MPUnit &result)
{
	MPUnit r(2 * base.length + 1);
	r = 1;
	bool one = true;
